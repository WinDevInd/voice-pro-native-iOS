...
export interface SpeechRecognitionPlugin {
    ...
    /**
     * Called with the normalized microphone volume while iOS speech capture is active.
     */
    addListener(eventName: 'volumeLevel', listenerFunc: (data: {
        level: number;
    }) => void): Promise<PluginListenerHandle>;
    /**
     * Called when native speech recognition stops because of an error.
     */
    addListener(eventName: 'speechError', listenerFunc: (data: {
        message: string;
    }) => void): Promise<PluginListenerHandle>;
    /**
